...
//-------------------------------------------------------------------------
// R8kVolRen3D_sav.cpp: implementation of the R8kVolRen3D_sav class.
//
//////////////////////////////////////////////////////////////////////
...
R8kVolRen3D_sav::R8kVolRen3D_sav()
{
	go = 0;
	fsNames = 0;
	pbfsNames = 0;
...
void R8kVolRen3D_sav::renderVolume()
{
...
//======================================================= Get Model View
//======================================================================
void R8kVolRen3D_sav::getModelView(float mv[16])
{
  float  m1[16];
  identityMatrix(m1);
...
void R8kVolRen3D_sav::renderBricks(GLdouble mv[16], GLfloat axis[4])
{
...
//========================================================== Setup Clips
//======================================================================
void R8kVolRen3D_sav::setupClips(int vol, float vo[8][3], float tx[8][3])
{
...
//============================================================ drawClips
//======================================================================
void R8kVolRen3D_sav::drawClip(int vol, float dv, float wmv[16])
{
#if 1
	if((!gluvv.clip.ortho)){
		switch(gluvv.dmode){
...
//======================================================================
void R8kVolRen3D_sav::renderSlice(float c[4][3], int vol)
{
...
//=========================================================== resetClips
//======================================================================
void R8kVolRen3D_sav::resetClips()
{
...
//======================================================== Setup Shaders
//======================================================================
void R8kVolRen3D_sav::setupTexUs()
{
#if 1
...
void R8kVolRen3D_sav::resetTexUs()
{
...
void R8kVolRen3D_sav::setupFragShade(float ftb)
{
	glEnable(GL_FRAGMENT_SHADER_ATI);
...
void R8kVolRen3D_sav::resetFragShade()
{
...
//====================================================== Set Up P-Buffer
//======================================================================
void R8kVolRen3D_sav::setupPBuff()       //prepare pbuffer for rendering
{
...
void R8kVolRen3D_sav::resetPBuff()              //reset pbuffer
{
	pbuff->MakeCurrent(); {
		resetFragShade();
		resetTexUs();
...
void R8kVolRen3D_sav::render3DVA(float sampleFrequency,  //how many samples per voxel
...
//standard renderer
inline void R8kVolRen3D_sav::volSlice(int edges, 
											             float tc[6][3], 
											             float pc[6][3],
											             int   order[6])
{
...
//render with shadows ===================================
inline void R8kVolRen3D_sav::volShadow(int edges,
																		 float tc[6][3],
																		 float pc[6][3],
																		 int   order[6],
																		 float ltxf[16],
																		 ...
																		 double mv[16])
{
	...
	pbuff->MakeCurrent();{
	  ...
	  //glDisable(GL_TEXTURE_3D);
#if 1
		// move constants to the shader!!
#if 1
//		float dx = 1/(float)gluvv.light.buffsz[0]*1/gluvv.volren.sampleRate;// * (rand()/(float)RAND_MAX * 2 - 1);
//		float dy = 1/(float)gluvv.light.buffsz[1]*1/gluvv.volren.sampleRate;// * (rand()/(float)RAND_MAX * 2 - 1);
#else
//		float dx = 0;//1/(float)gluvv.light.buffsz[0];
//		float dy = 0;//1/(float)gluvv.light.buffsz[1];
#endif
//		float cons0[4] = {dx* (rand()/(float)RAND_MAX * 2 - 1), dy* (rand()/(float)RAND_MAX * 2 - 1), 0, 1.0/4.0};
//		float cons1[4] = {-dx* (rand()/(float)RAND_MAX * 2 - 1), dy* (rand()/(float)RAND_MAX * 2 - 1), 0, 0};
//		glSetFragmentShaderConstantATI(GL_CON_0_ATI, cons0);
//		glSetFragmentShaderConstantATI(GL_CON_1_ATI, cons1);
		...
		glBegin(GL_POLYGON); //draw slice and texture map it
		{
			for(int j=0; j< edges; ++j){
				...
				glMultiTexCoord2fvARB(GL_TEXTURE3_ARB,lc[order[j]]);
//				glMultiTexCoord2fARB(GL_TEXTURE4_ARB,lc[order[j]][0]-dx* (rand()/(float)RAND_MAX * 2 - 1),lc[order[j]][1]-dy* (rand()/(float)RAND_MAX * 2 - 1));
//				glMultiTexCoord2fARB(GL_TEXTURE5_ARB,lc[order[j]][0]+dx* (rand()/(float)RAND_MAX * 2 - 1),lc[order[j]][1]-dy* (rand()/(float)RAND_MAX * 2 - 1));
    ...
    glTexParameteri(GL_TEXTURE_CUBE_MAP_ARB, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
#if 0
		wglBindTexImageARB(pbuff->buffer, WGL_FRONT_LEFT_ARB, NULL);
...
inline int R8kVolRen3D_sav::intersect(
...
void R8kVolRen3D_sav::renderSlice()
{
#if 0
	...
	GlErr("R8kVolRen3D_sav", "Draw Slice");	
#endif
}
...
void R8kVolRen3D_sav::createVolumes(int init)
{
	if(!glTexImage3DEXT){
		go = 0;
		return;
...
//======================================================= Create Bricks
//======================================================================
void R8kVolRen3D_sav::createBricks(int init)
{
	if(init){
	...
	for(int v=0; v<gluvv.mv->numSubVols; ++v){
		...
		case GDM_VGH:
			...
			break;
		default:
			cerr << "Error R8kVolRen3D_sav::createBricks(), data type unknown" << endl;
			return;
		}
		...
		glFlush();
		GlErr("R8kVolRen3D_sav", "Create Texture - Shade Texture");
		cerr << "Done" << endl;
...
//=================================================== load 1 byte volume
//======================================================================
void R8kVolRen3D_sav::loadTex1B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		...
		glFlush();
		GlErr("R8kVolRen3D_sav", "Create Texture - Data Texture");	
		cerr << "Done" << endl;	
}

//=================================================== load 2 byte volume
//======================================================================
void R8kVolRen3D_sav::loadTex2B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		cerr << " Down Loading Data Texture " << name << " ... ";
		glEnable(GL_TEXTURE_3D);
		glBindTexture(GL_TEXTURE_3D, name);
...
//=================================================== load 4 byte volume
//======================================================================
void R8kVolRen3D_sav::loadTex4B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		cerr << " Down Loading Data Texture " << name << " ... ";
		glEnable(GL_TEXTURE_3D);
		glBindTexture(GL_TEXTURE_3D, name);
...
void R8kVolRen3D_sav::create2DDepTex()
{
	//------------- load gb deptex --------------------
...
//========================================================= load Dep tex
//======================================================================
void R8kVolRen3D_sav::loadDepTex(unsigned char *dtex)
{
	GlErr("R8kVolRen3D_sav", "before load dep tex 1");
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	GlErr("R8kVolRen3D_sav", "load dep tex 1");
}

void R8kVolRen3D_sav::loadDepTex(unsigned char *dtex, unsigned int name)
{
	glEnable(GL_TEXTURE_2D);
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	GlErr("R8kVolRen3D_sav", "load dep tex 2");

}

//==================================================== create Shadow tex
//======================================================================
void R8kVolRen3D_sav::createShadowTex()  //creates the shadow texture
{
...
//======================================================================
void R8kVolRen3D_sav::createCubeTex()    //creates the cube map for shading
{
...
void R8kVolRen3D_sav::loadCubeFace(unsigned char *tex, unsigned int face, int sz)
{
		glTexImage2D(face,
			0,
			GL_RGBA8,
...
void R8kVolRen3D_sav::loadCubeTex(int good)      //updates cube map for shading
{
	int csz = gluvv.light.csz;
	...
	if(!good){
		pbuff->MakeCurrent(); { //render points to pbuffer
			...
			GlErr("R8kVolRen3D_sav::loadCubeTex", " begin");
			...
			for(int cf=0; cf<6; ++cf){ //for each cube face (cf)
				GlErr("R8kVolRen3D_sav::loadCubeTex", " render loop start");
				...
				GlErr("R8kVolRen3D_sav::loadCubeTex", " render loop normals");
				...
				glCopyTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + cf, 
					0, 0, 0, 0, 0, csz, csz);
				glFlush();
				GlErr("R8kVolRen3D_sav::loadCubeTex", " render loop copy");
				glBindTexture(GL_TEXTURE_CUBE_MAP_ARB, 0);			
				glDisable(GL_TEXTURE_CUBE_MAP_ARB);
				GlErr("R8kVolRen3D_sav::loadCubeTex", " render loop end");
			...
			glBindTexture(GL_TEXTURE_2D, 0);
			glDisable(GL_TEXTURE_2D);
			GlErr("R8kVolRen3D_sav::loadCubeTex", " light view");
#endif
			glMatrixMode(GL_PROJECTION);
			glPopMatrix();
			glMatrixMode(GL_MODELVIEW);
			...
			glDisable(GL_LIGHTING);
			GlErr("R8kVolRen3D_sav::loadCubeTex", " end");
		} MakeCurrent();
...
void R8kVolRen3D_sav::scaleAlpha(unsigned char *dout, float nsr, float osr, unsigned char *din)
{
	if(nsr == osr) return;
...
void R8kVolRen3D_sav::copyScale(unsigned char *dout, float sr, unsigned char *din)
{
	int dsx = gluvv.tf.ptexsz[0];
	int dsy = gluvv.tf.ptexsz[1];
...
void R8kVolRen3D_sav::MakeCurrent()
{
...
void R8kVolRen3D_sav::createFragShades(unsigned int *&names) // create and update fragment shaders
{
	if(!names){
