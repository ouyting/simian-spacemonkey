...
//-------------------------------------------------------------------------
// R8kVolRen3D_r2t.cpp: implementation of the R8kVolRen3D_r2t class.
...
R8kVolRen3D_r2t::R8kVolRen3D_r2t()
{
	go = 0;
	fsNames = 0;
	pbfsNames = 0;
...
void R8kVolRen3D_r2t::init()
{
    ...
    {
			pbuff->Initialize(true, true); //create a shared pbuffer, r2t
			pbuff->MakeCurrent(); 
      {   
	...
	cerr << "Using the render to texture Shadow Renderer!!!!!!!" << endl;
}

//================================================================= DRAW
//======================================================================

void R8kVolRen3D_r2t::draw()
{
	if(!go) return;
...
void R8kVolRen3D_r2t::renderVolume()
{
...
//======================================================= Get Model View
//======================================================================
void R8kVolRen3D_r2t::getModelView(float mv[16])
{
  float  m1[16];
  identityMatrix(m1);
...
void R8kVolRen3D_r2t::renderBricks(GLdouble mv[16], GLfloat axis[4])
{
...
//======================================================================
//======================================================================
void R8kVolRen3D_r2t::setupClips(int vol, float vo[8][3], float tx[8][3])
{
...
//======================================================================
//======================================================================
void R8kVolRen3D_r2t::drawClip(int vol, float dv, float wmv[16])
{
#if 1
	if((!gluvv.clip.ortho)){
		switch(gluvv.dmode){
...
//======================================================================
void R8kVolRen3D_r2t::renderSlice(float c[4][3], int vol)
{
...
//=========================================================== resetClips
//======================================================================
void R8kVolRen3D_r2t::resetClips()
{
...
//======================================================================
//======================================================================
void R8kVolRen3D_r2t::setupTexUs()
{
#if 1
...
void R8kVolRen3D_r2t::resetTexUs()
{
...
void R8kVolRen3D_r2t::setupFragShade(float ftb)
{
	glEnable(GL_FRAGMENT_SHADER_ATI);
...
void R8kVolRen3D_r2t::resetFragShade()
{
...
//======================================================================
//======================================================================
void R8kVolRen3D_r2t::setupPBuff()       //prepare pbuffer for rendering
{
...
void R8kVolRen3D_r2t::resetPBuff()              //reset pbuffer
{
	pbuff->MakeCurrent(); {
		resetFragShade();
		resetTexUs();
...
void R8kVolRen3D_r2t::render3DVA(float sampleFrequency,  //how many samples per voxel
...
//standard renderer
inline void R8kVolRen3D_r2t::volSlice(int edges, 
											             float tc[6][3], 
											             float pc[6][3],
											             int   order[6])
{
...
//render with shadows ===================================
inline void R8kVolRen3D_r2t::volShadow(int edges,
																		 float tc[6][3],
																		 float pc[6][3],
																		 int   order[6],
																		 float ltxf[16],
  ...
  //copy the current alpha from the pbuffer, and draw the latest
      ...
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		...
		glBegin(GL_POLYGON); //draw slice and texture map it
		{
			for(int j=0; j< edges; ++j){
				...
				glMultiTexCoord2fARB(GL_TEXTURE3_ARB,lc[order[j]][0], lc[order[j]][1]);
				//the coords in screen space
				glVertex2f(lv[order[j]][0], lv[order[j]][1]);
			}
		}
	...
	glBegin(GL_POLYGON); //draw slice and texture map it
	{
		for(int j=0; j< edges; ++j){
			...
			glMultiTexCoord2fARB(GL_TEXTURE0_ARB,lc[order[j]][0], lc[order[j]][1]);
			glMultiTexCoord3fvARB(GL_TEXTURE2_ARB,tc[order[j]]);
			glMultiTexCoord2fARB(GL_TEXTURE5_ARB,lc[order[j]][0], lc[order[j]][1]);
...
inline int R8kVolRen3D_r2t::intersect(
...
void R8kVolRen3D_r2t::renderSlice()
{
#if 0
	...
	GlErr("R8kVolRen3D_r2t", "Draw Slice");	
#endif
}
...
void R8kVolRen3D_r2t::createVolumes(int init)
{
	if(!glTexImage3DEXT){
		go = 0;
		return;
...
//======================================================================
//======================================================================
void R8kVolRen3D_r2t::createBricks(int init)
{
	if(init){
	...
	for(int v=0; v<gluvv.mv->numSubVols; ++v){
		...
		case GDM_VGH:
			...
			break;
		default:
			cerr << "Error R8kVolRen3D_r2t::createBricks(), data type unknown" << endl;
			return;
		}
		...
		glFlush();
		GlErr("R8kVolRen3D_r2t", "Create Texture - Shade Texture");
		cerr << "Done" << endl;
...
//=================================================== load 1 byte volume
//======================================================================
void R8kVolRen3D_r2t::loadTex1B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		...
		glFlush();
		GlErr("R8kVolRen3D_r2t", "Create Texture - Data Texture");	
		cerr << "Done" << endl;	
}

//=================================================== load 2 byte volume
//======================================================================
void R8kVolRen3D_r2t::loadTex2B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		cerr << " Down Loading Data Texture " << name << " ... ";
		glEnable(GL_TEXTURE_3D);
		glBindTexture(GL_TEXTURE_3D, name);
...
//=================================================== load 4 byte volume
//======================================================================
void R8kVolRen3D_r2t::loadTex4B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		cerr << " Down Loading Data Texture " << name << " ... ";
		glEnable(GL_TEXTURE_3D);
		glBindTexture(GL_TEXTURE_3D, name);
...
void R8kVolRen3D_r2t::create2DDepTex()
{
	//------------- load gb deptex --------------------
...
//======================================================================
//======================================================================
void R8kVolRen3D_r2t::loadDepTex(unsigned char *dtex)
{
	GlErr("R8kVolRen3D_r2t", "before load dep tex 1");
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	GlErr("R8kVolRen3D_r2t", "load dep tex 1");
}

void R8kVolRen3D_r2t::loadDepTex(unsigned char *dtex, unsigned int name)
{
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	GlErr("R8kVolRen3D_r2t", "load dep tex 2");
...
//======================================================================
//======================================================================
void R8kVolRen3D_r2t::createShadowTex()  //creates the shadow texture
{
...
//======================================================================
//======================================================================
void R8kVolRen3D_r2t::createNoiseTex(int sx, int sy, int sz)   //creates the noise texture
{
...
//======================================================================
//======================================================================
void R8kVolRen3D_r2t::createCubeTex()    //creates the cube map for shading
{
...
void R8kVolRen3D_r2t::loadCubeFace(unsigned char *tex, unsigned int face, int sz)
{
		glTexImage2D(face,
			0,
			GL_RGBA8,
...
void R8kVolRen3D_r2t::loadCubeTex(int good)      //updates cube map for shading
{
	int csz = gluvv.light.csz;
	...
	if(!good){
		pbuff->MakeCurrent(); { //render points to pbuffer
			...
			GlErr("R8kVolRen3D_r2t::loadCubeTex", " begin");
			...
			for(int cf=0; cf<6; ++cf){ //for each cube face (cf)
				GlErr("R8kVolRen3D_r2t::loadCubeTex", " render loop start");
				...
				GlErr("R8kVolRen3D_r2t::loadCubeTex", " render loop normals");
				...
				glCopyTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + cf, 
					0, 0, 0, 0, 0, csz, csz);
				glFlush();
				GlErr("R8kVolRen3D_r2t::loadCubeTex", " render loop copy");
				glBindTexture(GL_TEXTURE_CUBE_MAP_ARB, 0);			
				glDisable(GL_TEXTURE_CUBE_MAP_ARB);
				GlErr("R8kVolRen3D_r2t::loadCubeTex", " render loop end");
			...
			glBindTexture(GL_TEXTURE_2D, 0);
			glDisable(GL_TEXTURE_2D);
			GlErr("R8kVolRen3D_r2t::loadCubeTex", " light view");
#endif
			glMatrixMode(GL_PROJECTION);
			glPopMatrix();
			glMatrixMode(GL_MODELVIEW);
			...
			glDisable(GL_LIGHTING);
			GlErr("R8kVolRen3D_r2t::loadCubeTex", " end");
		} MakeCurrent();
...
void R8kVolRen3D_r2t::scaleAlpha(unsigned char *dout, float nsr, float osr, unsigned char *din)
{
	if(nsr == osr) return;
...
void R8kVolRen3D_r2t::copyScale(unsigned char *dout, float sr, unsigned char *din)
{
	int dsx = gluvv.tf.ptexsz[0];
	int dsy = gluvv.tf.ptexsz[1];
...
void R8kVolRen3D_r2t::MakeCurrent()
{
...
void R8kVolRen3D_r2t::createFragShades(unsigned int *&names) // create and update fragment shaders
{
	if(!names){
...
void R8kVolRen3D_r2t::createFragShadow(unsigned int *&names) // create and update fragment shaders
{
	if(!names){
		names = new unsigned int[20];
		for(int i=0; i<20; ++i) names[i] = -1;
...
void R8kVolRen3D_r2t::createFragNoise(unsigned int *&names) // create and update fragment shaders
{
	if(!names){
		names = new unsigned int[20];
		for(int i=0; i<20; ++i) names[i] = -1;
