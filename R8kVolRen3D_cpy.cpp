...
//-------------------------------------------------------------------------
// R8kVolRen3D_cpy.cpp: implementation of the R8kVolRen3D_cpy class.
//   This is the version that uses copies
//////////////////////////////////////////////////////////////////////
...
R8kVolRen3D_cpy::R8kVolRen3D_cpy()
{
	go = 0;
	fsNames = 0;
	pbfsNames = 0;
...
void R8kVolRen3D_cpy::init()
{
    ...
    createFragShades(fsNames);
		createCubeTex();
    createNoiseTex(32,32,32);

    //light transport pbuffer
    ...
    {
			pbuff->Initialize(true); //create a shared pbuffer
			pbuff->MakeCurrent(); 
      {   
				...
				gluLookAt(0,0,2,
					        0,0,0,
									0,1,0);
				createFragShades(pbfsNames);
			} MakeCurrent();
			cerr << "PBuffer intialization succeded" << endl;
		}

    //data cache and perturbation pbuffer
    ...
    {
			pbuff2->Initialize(true); //create a shared pbuffer
			pbuff2->MakeCurrent(); 
      {   
				glClearColor( 0.0, 0.0, 0.0, 0.0 );			
				glMatrixMode( GL_PROJECTION );
...
void R8kVolRen3D_cpy::renderVolume()
{
  ...
  srand(10); //seed rand for jittering
	...
	if(gluvv.volren.scaleAlphas){
      ...
      if(gluvv.volren.loadTLUT){  //now load the transfer function
        ...
        if(gluvv.volren.deptex3)
        {
          loadDepTex(gluvv.volren.deptex3, gluvv.volren.deptex3Name);
          cerr << "loaded auxilary transfer function" << endl;
        }
			}
  ...
  {  //just do gamma scale, don't update for the sample rate (for testing purposes)
    if(gluvv.volren.loadTLUT){
      ...
      {
        loadDepTex(gluvv.volren.deptex3, gluvv.volren.deptex3Name);
        cerr << "loading auxilary transfer function" << endl;
      }
      gluvv.volren.loadTLUT = 0;
		}
	}
  ...
  //-------------- do dot product with clip and view dir
...
#if 0  //need this for the perturbation stuff
    GLfloat tmpmv[16];
		getModelView(tmpmv);
		GLfloat invmv[16];
    ...
    float cons3[4] = {gluvv.pert.weights[0],
			                gluvv.pert.weights[1],
											gluvv.pert.weights[2],
											gluvv.pert.weights[3]};
		glSetFragmentShaderConstantATI(GL_CON_3_ATI, cons3);		
		
    // --------------------- end set up attinuation parameters ------------

		//-------------- draw clip slices --------------------------------------
		if(axis[3] <= 0){ //only do this if front to back
			glEnable(GL_BLEND);
	...
	if(gluvv.light.showView){
		pbuff->MakeCurrent();{
			...
			glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 
				                  0, 0,
					                gluvv.light.buffsz[0], gluvv.light.buffsz[1]);
    ...
    glEnable(GL_TEXTURE_3D);
		glBindTexture(GL_TEXTURE_3D, noiseName);
		glBegin(GL_QUADS);{
			glTexCoord2f(0,0);
			glVertex3f(-1,-1,0);
			glTexCoord2f(1,0);
...
//======================================================= Get Model View
//======================================================================
void R8kVolRen3D_cpy::getModelView(float mv[16])
{
  float  m1[16];
  identityMatrix(m1);
...
void R8kVolRen3D_cpy::renderBricks(GLdouble mv[16], GLfloat axis[4])
{
...
//======================================================================
//======================================================================
void R8kVolRen3D_cpy::setupClips(int vol, float vo[8][3], float tx[8][3])
{
...
//======================================================================
//======================================================================
void R8kVolRen3D_cpy::drawClip(int vol, float dv, float wmv[16])
{
#if 1
	if((!gluvv.clip.ortho)){
		switch(gluvv.dmode){
...
//======================================================================
void R8kVolRen3D_cpy::renderSlice(float c[4][3], int vol)
{
...
//=========================================================== resetClips
//======================================================================
void R8kVolRen3D_cpy::resetClips()
{
...
//======================================================================
//======================================================================
void R8kVolRen3D_cpy::setupTexUs()
{
#if 1
		...
		glActiveTexture(GL_TEXTURE4_ARB);{ //cube map
		...
		glActiveTexture(GL_TEXTURE5_ARB);{ //shadow
			glEnable(GL_TEXTURE_2D);
			glBindTexture(GL_TEXTURE_2D, pbuffName);
		}
		GlErr("set up texture units", "after texture4");
...
void R8kVolRen3D_cpy::resetTexUs()
{
	//NOTE: you must completely un-bind any texture which will be copied to later
#if 1
		glActiveTexture(GL_TEXTURE5_ARB); { //not used
				glDisable(GL_TEXTURE_CUBE_MAP_ARB);
				glDisable(GL_TEXTURE_2D);
				glDisable(GL_TEXTURE_3D);
				glBindTexture(GL_TEXTURE_1D, 0);
		}
		glActiveTexture(GL_TEXTURE4_ARB); { //not used
...
void R8kVolRen3D_cpy::setupFragShade(float ftb)
{
	glEnable(GL_FRAGMENT_SHADER_ATI);
	if(ftb <= 0){ //back to front
		//shaded
...
void R8kVolRen3D_cpy::resetFragShade()
{
...
//======================================================================
//======================================================================
void R8kVolRen3D_cpy::setupPBuff()       //prepare pbuffer for rendering
{
	...
	pbuff->MakeCurrent(); {
		...
		GlErr("r8kvolren", "setupPbub()-start");
		...
		if(gluvv.volren.sampleRate == gluvv.volren.goodSamp){
			xscale = gluvv.light.gShadowQual * .8;
			yscale = gluvv.light.gShadowQual * .8;
		} else {
			xscale = gluvv.light.iShadowQual * .8;
			yscale = gluvv.light.iShadowQual * .8;
		}

		//This sets the projection to ortho with screen params = [0..buff-size]
		glMatrixMode( GL_PROJECTION );{  //figure out the window size
			glLoadIdentity();
			glOrtho(0,gluvv.light.buffsz[0],0,gluvv.light.buffsz[1],1,3);
		}
		//eye = [0,0,2], lookat = [0,0,0], up = [0,1,0]
		glMatrixMode(GL_MODELVIEW);{
			glLoadIdentity();
			gluLookAt(0,0,2,
								0,0,0,
		...
		if(gluvv.light.softShadow){ //soft shadows
			glDisable(GL_BLEND);      //blend in fragment shader
		} else {
		  glEnable(GL_BLEND);       //otherwise blend in frame buffer
		  glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
		}

		//set some gl state
		glDisable(GL_LIGHTING); //lighting makes it look bad
		glDisable(GL_DEPTH_TEST); //no depth test
		glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);	//clear the buffer
		//setup texture units

		//clear the shadow texture
		glActiveTexture(GL_TEXTURE4_ARB); { // shadow texture
			glEnable(GL_TEXTURE_2D);
			glBindTexture(GL_TEXTURE_2D, 0);
		} //end texture 2
		glActiveTexture(GL_TEXTURE5_ARB); { // shadow texture
			glEnable(GL_TEXTURE_2D);
			glBindTexture(GL_TEXTURE_2D, 0);
		} //end texture 2
		glActiveTexture(GL_TEXTURE0_ARB);{  // bind shadow name for copying
		...
		glBindTexture(GL_TEXTURE_2D, 0);
		glDisable(GL_TEXTURE_2D);
		//end clear the shadow texture

		GlErr("r8kvolren - setup pbuff", "before set tex units");
		...
		GlErr("pbuffer::set up shader", "after texture2");
		...
		glEnable(GL_FRAGMENT_SHADER_ATI);
		if(gluvv.light.softShadow)
			glBindFragmentShaderATI(pbfsNames[19]);
		else
  ...
  pbuff2->MakeCurrent();
  {
    ...
    glEnable(GL_FRAGMENT_SHADER_ATI);
    {
      if(gluvv.pert.weights[3] > 0)
      {
        glBindFragmentShaderATI(pbfnNames[4]);
      }
      else if(gluvv.pert.weights[2] > 0)
      {
        glBindFragmentShaderATI(pbfnNames[3]);
      }
      else if(gluvv.pert.weights[1] > 0)
      {
        glBindFragmentShaderATI(pbfnNames[2]);
        cerr << "using noise 2" << endl;
      }
      else if(gluvv.pert.weights[0] > 0)
      {
        glBindFragmentShaderATI(pbfnNames[1]);
        cerr << "using noise 1" << endl;
      }
      else 
      {
        glBindFragmentShaderATI(pbfnNames[0]);
      }
    }
...
void R8kVolRen3D_cpy::resetPBuff()              //reset pbuffer
{
	pbuff->MakeCurrent(); {
		resetFragShade();
		resetTexUs();
...
void R8kVolRen3D_cpy::render3DVA(float sampleFrequency,  //how many samples per voxel
...
//standard renderer
inline void R8kVolRen3D_cpy::volSlice(int edges, 
											             float tc[6][3], 
											             float pc[6][3],
											             int   order[6])
{
...
//render with shadows ===================================
inline void R8kVolRen3D_cpy::volShadow(int edges,
																		 float tc[6][3],
																		 float pc[6][3],
																		 int   order[6],
																		 float ltxf[16],
																		 ...
																		 double mv[16])
{
  ...
  //Take this crap out, it didn't work
  static float lastJitter[2] = {0, 0};
  float thisJitter[2] = { (rand()/(float)RAND_MAX -.5), (rand()/(float)RAND_MAX -.5)};
  float tjnorm = 1.0/sqrt(thisJitter[0]*thisJitter[0] + thisJitter[1]*thisJitter[1]);
  thisJitter[0] *= tjnorm/9023;
  thisJitter[1] *= tjnorm/9023;
  ...
  thisJitter[1] = 0;
	...
	for(int i=0; i<edges; ++i){
		...
		lc[idx][0] = (lv[idx][0] + xbias)/(2*xbias)*xscale;
		lc[idx][1] = (lv[idx][1] + ybias)/(2*ybias)*yscale;
    ...
    glColor4f(1,1,1,1);
		glBegin(GL_POLYGON); //draw slice and texture map it
		{
			for(int j=0; j< edges; ++j){
				//the incomming slice
				glMultiTexCoord3fvARB(GL_TEXTURE0_ARB,tc[order[j]]);
				glMultiTexCoord3fARB(GL_TEXTURE1_ARB,tc[order[j]][0]*gluvv.pert.scales[0], 
                                             tc[order[j]][1]*gluvv.pert.scales[0],
                                             tc[order[j]][2]*gluvv.pert.scales[0]);
        glMultiTexCoord3fARB(GL_TEXTURE2_ARB,tc[order[j]][0]*gluvv.pert.scales[1], 
                                             tc[order[j]][1]*gluvv.pert.scales[1],
                                             tc[order[j]][2]*gluvv.pert.scales[1]);
				//the coords in screen space
				//glVertex2f(lv[order[j]][0] + thisJitter[0], lv[order[j]][1] + thisJitter[1]);
				glVertex2f(lv[order[j]][0], lv[order[j]][1]);
			}
		}
		glEnd();
    glDisable(GL_FRAGMENT_SHADER_ATI);
    ...
    glActiveTexture(GL_TEXTURE0_ARB);{
      ...
      glEnable(GL_TEXTURE_2D);
		...
		if(((maxx-minx)>1) && ((maxy-miny)>1)){
			//glCopyTexSubImage2D(GL_TEXTURE_2D, 0, (int)minx + thisJitter[0], (int)miny + thisJitter[1], 
			//	                  (int)minx + lastJitter[0], (int)miny + lastJitter[1],
  ...
  //copy the current alpha from the pbuffer, and draw the latest
	pbuff->MakeCurrent();{
...
#if 1
		//you have to completely unbind a texture before copying
		//glActiveTexture(GL_TEXTURE2_ARB); { // shadow texture
		//	glEnable(GL_TEXTURE_2D);
		//	glBindTexture(GL_TEXTURE_2D, 0);
		//} //end texture 2
		glActiveTexture(GL_TEXTURE3_ARB); { // shadow texture
			glEnable(GL_TEXTURE_2D);
			glBindTexture(GL_TEXTURE_2D, 0);
		} //end texture 3
		glActiveTexture(GL_TEXTURE4_ARB); { // shadow texture
		...
		if(((maxx-minx)>1) && ((maxy-miny)>1)){
			glCopyTexSubImage2D(GL_TEXTURE_2D, 0, (int)minx + lastJitter[0], (int)miny + lastJitter[1], 
				                  (int)minx + lastJitter[0], (int)miny + lastJitter[1],
					                (int)(maxx-minx), (int)(maxy-miny));
		}
#endif
    ...
    glActiveTexture(GL_TEXTURE2_ARB); { // auxilary tf
			glEnable(GL_TEXTURE_2D);
			glBindTexture(GL_TEXTURE_2D, gluvv.volren.deptex3Name);    
    } //end texture 3
		glActiveTexture(GL_TEXTURE3_ARB); { // shadow texture
      ...
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		...
		glBegin(GL_POLYGON); //draw slice and texture map it
		{
			for(int j=0; j< edges; ++j){
				//the incomming slice
				//glMultiTexCoord3fvARB(GL_TEXTURE0_ARB,tc[order[j]]);
				//glMultiTexCoord2fARB(GL_TEXTURE0_ARB,lc[order[j]][0] + thisJitter[0], lc[order[j]][1] + thisJitter[1]);
				glMultiTexCoord2fARB(GL_TEXTURE0_ARB,lc[order[j]][0], lc[order[j]][1]);
				//the slice from the shadow buffer
				glMultiTexCoord2fARB(GL_TEXTURE3_ARB,lc[order[j]][0] + lastJitter[0], lc[order[j]][1] + lastJitter[1]);
				//the coords in screen space
				glVertex2f(lv[order[j]][0] + thisJitter[0], lv[order[j]][1] + thisJitter[1]);
			}
		}
	...
	glBegin(GL_POLYGON); //draw slice and texture map it
	{
		for(int j=0; j< edges; ++j){
			...
			glMultiTexCoord2fARB(GL_TEXTURE0_ARB,lc[order[j]][0], lc[order[j]][1]);
			glMultiTexCoord3fvARB(GL_TEXTURE2_ARB,tc[order[j]]);
			glMultiTexCoord2fARB(GL_TEXTURE5_ARB,lc[order[j]][0] + lastJitter[0], lc[order[j]][1] + lastJitter[0]);
  ...
  lastJitter[0] = thisJitter[0];
  lastJitter[1] = thisJitter[1];
...
inline int R8kVolRen3D_cpy::intersect(
...
void R8kVolRen3D_cpy::renderSlice()
{
#if 0
	...
	GlErr("R8kVolRen3D_cpy", "Draw Slice");	
#endif
}
...
void R8kVolRen3D_cpy::createVolumes(int init)
{
	if(!glTexImage3DEXT){
		go = 0;
		return;
...
//======================================================================
//======================================================================
void R8kVolRen3D_cpy::createBricks(int init)
{
	if(init){
	...
	for(int v=0; v<gluvv.mv->numSubVols; ++v){
		...
		case GDM_VGH:
			...
			break;
		default:
			cerr << "Error R8kVolRen3D_cpy::createBricks(), data type unknown" << endl;
			return;
		}
		...
		glFlush();
		GlErr("R8kVolRen3D_cpy", "Create Texture - Shade Texture");
		cerr << "Done" << endl;
...
//=================================================== load 1 byte volume
//======================================================================
void R8kVolRen3D_cpy::loadTex1B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		...
		glFlush();
		GlErr("R8kVolRen3D_cpy", "Create Texture - Data Texture");	
		cerr << "Done" << endl;	
}

//=================================================== load 2 byte volume
//======================================================================
void R8kVolRen3D_cpy::loadTex2B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		cerr << " Down Loading Data Texture " << name << " ... ";
		glEnable(GL_TEXTURE_3D);
		glBindTexture(GL_TEXTURE_3D, name);
...
//=================================================== load 4 byte volume
//======================================================================
void R8kVolRen3D_cpy::loadTex4B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		cerr << " Down Loading Data Texture " << name << " ... ";
		glEnable(GL_TEXTURE_3D);
		glBindTexture(GL_TEXTURE_3D, name);
...
void R8kVolRen3D_cpy::create2DDepTex()
{
	//------------- load gb deptex --------------------
  ...
  cerr << " auxilary dep tex = " << gluvv.volren.deptex3Name << endl;
	...
	for(int j=0; j<dsy; ++j){
      ...
      stf[j*dsx*4 + k*4 + 0] = 255;
      stf[j*dsx*4 + k*4 + 1] = 255;
      stf[j*dsx*4 + k*4 + 2] = 255;
      stf[j*dsx*4 + k*4 + 3] = 255; 
		}
	}
	...
	loadDepTex(gluvv.volren.deptex3, gluvv.volren.deptex3Name); //load auxilary tf for evaluation
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	glTexImage2D(GL_TEXTURE_2D,
			...
			deptex);
	glFlush();
#if 1 //not necessary, already loaded it
	//Load the shadows (auxilary) tf (for viewing)
	glBindTexture(GL_TEXTURE_2D, gluvv.volren.deptex3Name);

  cerr << " loading auxilary dep tex = " << gluvv.volren.deptex3Name << endl;
...
//======================================================================
//======================================================================
void R8kVolRen3D_cpy::loadDepTex(unsigned char *dtex)
{
	GlErr("R8kVolRen3D_cpy", "before load dep tex 1");
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	GlErr("R8kVolRen3D_cpy", "load dep tex 1");
}

void R8kVolRen3D_cpy::loadDepTex(unsigned char *dtex, unsigned int name)
{
  cerr << " Loading dep tex # " << name << endl;
	glEnable(GL_TEXTURE_2D);
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	GlErr("R8kVolRen3D_cpy", "load dep tex 2");
...
//======================================================================
//======================================================================
void R8kVolRen3D_cpy::createShadowTex()  //creates the shadow texture
{
...
//======================================================================
//======================================================================
void R8kVolRen3D_cpy::createNoiseTex(int sx, int sy, int sz)   //creates the noise texture
{
  cerr << "Creating Noise texture " << sx << " " << sy << " " << sz;


  unsigned char *ptdata = new unsigned char[sx*sy*sz*4];
  ...
  for(int i = 0; i<64; ++i){
    for(int j=0; j<64; ++j){
      for(int k=0; k<64; ++k){
        ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 0] = (px.PNoise3D(i/63.0*2, j/63.0*2, k/63.0*2)*.5 + .5) * 255;
        ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 1] = (py.PNoise3D(i/63.0*2, j/63.0*2, k/63.0*2)*.5 + .5) * 255;
        ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 2] = (pz.PNoise3D(i/63.0*2, j/63.0*2, k/63.0*2)*.5 + .5) * 255;
        ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 3] = (pw.PNoise3D(i/63.0*2, j/63.0*2, k/63.0*2)*.5 + .5) * 255;
      }
    }
  }
#else
  ...
  for(int i = 0; i<sz; ++i){
    for(int j=0; j<sy; ++j){
      for(int k=0; k<sx; ++k){
        ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 0] = ((rand()/(float)RAND_MAX *.5) + .5 + 1.0/512) * 255;
        ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 1] = ((rand()/(float)RAND_MAX *.5) + .5 + 1.0/512) * 255;;
        ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 2] = ((rand()/(float)RAND_MAX *.5) + .5 + 1.0/512) * 255;;
        ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 3] = ((rand()/(float)RAND_MAX *.5) + .5 + 1.0/512) * 255;;
      }
    }
  }

  for(int t=0; t<0; ++ t){  //brain dead blurring
    for(int i = 0; i<sz; ++i){
      for(int j=0; j<sy; ++j){
        for(int k=0; k<sx; ++k){
          int upx = (k + 1) %sx;
          int dnx = (k - 1) < 0 ? (sx-1) : (k - 1);
          int upy = (j + 1) %sy;
          int dny = (j - 1) < 0 ? (sy-1) : (j - 1);
          int upz = (i + 1) %sz;
          int dnz = (i - 1) < 0 ? (sz-1) : (i - 1);
          ptdata[i*sx*sy*4 + j*64*4 + k*4 + 0] = 
            (ptdata[upz*sx*sy*4 + j*sx*4 + k*4 + 0] + ptdata[dnz*sx*sy*4 + j*sx*4 + k*4 + 0] + 
            ptdata[i*sx*sy*4 + upy*sx*4 + k*4 + 0] + ptdata[i*sx*sy*4 + dny*sx*4 + k*4 + 0] +
            ptdata[i*sx*sy*4 + j*sx*4 + upx*4 + 0] + ptdata[i*sx*sy*4 + j*sx*4 + dnx*4 + 0] +
            ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 0]*10)/16.0;
          ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 1] = 
            (ptdata[upz*sx*sy*4 + j*sx*4 + k*4 + 1] + ptdata[dnz*sx*sy*4 + j*sx*4 + k*4 + 1] + 
            ptdata[i*sx*sy*4 + upy*sx*4 + k*4 + 1] + ptdata[i*sx*sy*4 + dny*sx*4 + k*4 + 1] +
            ptdata[i*sx*sy*4 + j*sx*4 + upx*4 + 1] + ptdata[i*sx*sy*4 + j*sx*4 + dnx*4 + 1] +
            ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 1]*10)/16.0;
          ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 2] = 
            (ptdata[upz*sx*sy*4 + j*sx*4 + k*4 + 2] + ptdata[dnz*sx*sy*4 + j*sx*4 + k*4 + 2] + 
            ptdata[i*sx*sy*4 + upy*sx*4 + k*4 + 2] + ptdata[i*sx*sy*4 + dny*sx*4 + k*4 + 2] +
            ptdata[i*sx*sy*4 + j*sx*4 + upx*4 + 2] + ptdata[i*sx*sy*4 + j*sx*4 + dnx*4 + 2] +
            ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 2]*10)/16.0;
          ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 3] = 
            (ptdata[upz*sx*sy*4 + j*sx*4 + k*4 + 3] + ptdata[dnz*sx*sy*4 + j*sx*4 + k*4 + 3] + 
            ptdata[i*sx*sy*4 + upy*sx*4 + k*4 + 3] + ptdata[i*sx*sy*4 + dny*sx*4 + k*4 + 3] +
            ptdata[i*sx*sy*4 + j*sx*4 + upx*4 + 3] + ptdata[i*sx*sy*4 + j*sx*4 + dnx*4 + 3] +
            ptdata[i*sx*sy*4 + j*sx*4 + k*4 + 3]*10)/16.0;
        }
      }
    }
  }
  ...
  glGenTextures(1, &noiseName);   //the perturbation texture
  ...
  glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	glFlush();
	glDisable(GL_TEXTURE_3D);
  GlErr("ATI copy shader", "after noise load");
  delete[] ptdata;
...
//======================================================================
//======================================================================
void R8kVolRen3D_cpy::createCubeTex()    //creates the cube map for shading
{
  ...
  glTexParameteri(GL_TEXTURE_CUBE_MAP_ARB, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	...
	for(int i=0; i<csz; ++i){ //z
		for(int j=0; j<csz; ++j){ //y
...
void R8kVolRen3D_cpy::loadCubeFace(unsigned char *tex, unsigned int face, int sz)
{
		glTexImage2D(face,
			0,
			GL_RGBA8,
...
void R8kVolRen3D_cpy::loadCubeTex(int good)      //updates cube map for shading
{
	int csz = gluvv.light.csz;

	if(!good){
		pbuff->MakeCurrent(); { //render points to pbuffer
			
			GlErr("R8kVolRen3D_cpy::loadCubeTex", " begin");
			
			glShadeModel(GL_SMOOTH);
			//set up lighting (diffuse & ambiant are red, specular is green)
			static float ambient[]             = {1, 1, 1, 1};
			static float diffuse[]             = {1, 1, 1, 1};
			static float specular[]            = {0, 1.0, 0, 1};
			static float front_mat_shininess[] = {30.0};
			static float front_mat_specular[]  = {0, gluvv.light.intens, 0, 1};
			static float front_mat_diffuse[]   = {gluvv.light.intens, 0, 0, 1};
			static float front_mat_ambient[]   = {.2, 0, 0, 1};
			static float lmodel_ambient[]      = {.2, 0, 0, 1};
			...
			float ltpos[4] = {0,0,0,1};
			copyV3(ltpos, gluvv.light.pos);
			scaleV3(10000, ltpos);
			negateV3(ltpos);
			...
			float dx = .5;//1/(2*gluvv.light.buffsz[0]);
			float dy = .5;//1/(2*gluvv.light.buffsz[1]);
			for(int cf=0; cf<6; ++cf){ //for each cube face (cf)
				GlErr("R8kVolRen3D_cpy::loadCubeTex", " render loop start");
				glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
				glColor4f(1,1,1,1);
				glBegin(GL_POINTS);{
					for(int i=0; i<csz; ++i){
						for(int j=0; j<csz; ++j){
							glNormal3fv(&gluvv.light.cubeKey[cf][i*csz*3 + j*3]);
							glVertex3f(i+dx,j+dy,0);
						}
					}
				} glEnd();
				GlErr("R8kVolRen3D_cpy::loadCubeTex", " render loop normals");
				
				glEnable(GL_TEXTURE_CUBE_MAP_ARB);
				glBindTexture(GL_TEXTURE_CUBE_MAP_ARB, gluvv.light.cubeName);
				glCopyTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + cf, 
					0, 0, 0, 0, 0, csz, csz);
				glFlush();
				GlErr("R8kVolRen3D_cpy::loadCubeTex", " render loop copy");
				glBindTexture(GL_TEXTURE_CUBE_MAP_ARB, 0);			
				glDisable(GL_TEXTURE_CUBE_MAP_ARB);
				GlErr("R8kVolRen3D_cpy::loadCubeTex", " render loop end");
			...
			glBindTexture(GL_TEXTURE_2D, 0);
			glDisable(GL_TEXTURE_2D);
			GlErr("R8kVolRen3D_cpy::loadCubeTex", " light view");
#endif
			glMatrixMode(GL_PROJECTION);
			glPopMatrix();
			glMatrixMode(GL_MODELVIEW);
			...
			glDisable(GL_LIGHTING);
			GlErr("R8kVolRen3D_cpy::loadCubeTex", " end");
		} MakeCurrent();

	} else { //this is the good load
		...
		glTexParameteri(GL_TEXTURE_CUBE_MAP_ARB, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_CUBE_MAP_ARB, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		for(unsigned int f=0; f<6; ++f){ //for each face
			for(int i=0; i<csz; ++i){ //for y
				for(int j=0; j<csz; ++j){ // for x
					float n[3];
...
void R8kVolRen3D_cpy::scaleAlpha(unsigned char *dout, float nsr, float osr, unsigned char *din)
{
	if(nsr == osr) return;
...
void R8kVolRen3D_cpy::copyScale(unsigned char *dout, float sr, unsigned char *din)
{
	int dsx = gluvv.tf.ptexsz[0];
	int dsy = gluvv.tf.ptexsz[1];
...
void R8kVolRen3D_cpy::MakeCurrent()
{
...
void R8kVolRen3D_cpy::createFragShades(unsigned int *&names) // create and update fragment shaders
{
	if(!names){
	...
	int thirdAxis = 0;
	switch(gluvv.dmode){ //third axis switch, to use or not to use???
		case GDM_VGH:
		case GDM_V1GH:
		case GDM_V2G:
		case GDM_V2GH:
	...
	for(int i=0; i<10; ++i){
		...
		glEnable(GL_FRAGMENT_SHADER_ATI);{ //The Shader 
			...
			glBeginFragmentShaderATI();{
				...
				if((i==4)||(i==5)||(i==6)||(i==7)||(i==9)||(1)){ //shading
				...
				if(thirdAxis){
#if 1   //just moving data to another register so we can keep 0 free
				...
				if((i == 4)||(i == 5)||(i==6)||(i==7)||(i==9)){ //shading transform
	...
	glEnable(GL_FRAGMENT_SHADER_ATI);{ //shadow buffer shader
      ...
      glColorFragmentOp1ATI(GL_MOV_ATI,     //move the datavalues for the auxilary tf
	...
	glEnable(GL_FRAGMENT_SHADER_ATI);{ //clipping plane shader
		glBindFragmentShaderATI(cpfsName);
		glBeginFragmentShaderATI();{
			//----- routing instructions ------
			//grab the tex coords for tex 0
...
void R8kVolRen3D_cpy::createFragShadow(unsigned int *&names) // create and update fragment shaders
{
	if(!names){
		names = new unsigned int[20];
		for(int i=0; i<20; ++i) names[i] = -1;
...
void R8kVolRen3D_cpy::createFragNoise(unsigned int *&names) // create and update fragment shaders
{
	if(!names){
		names = new unsigned int[20];
		for(int i=0; i<20; ++i) names[i] = -1;
  ...
  if(names[0] == -1)                             //no op shader
    names[0] = glGenFragmentShadersATI(1);
		
  glEnable(GL_FRAGMENT_SHADER_ATI);
  { //The Shader 
    glBindFragmentShaderATI(names[0]);
    glBeginFragmentShaderATI();
    {	
        glSampleMapATI(GL_REG_0_ATI, GL_TEXTURE0_ARB, GL_SWIZZLE_STR_ATI);
  ...
  //=================================================================
	//=============================================================== i
#if 1
  for(int i=1; i<4; ++i){   //the perturbation shaders
		if(names[i] == -1)
			names[i] = glGenFragmentShadersATI(1);
...
//  				glSampleMapATI(GL_REG_4_ATI, GL_TEXTURE4_ARB, GL_SWIZZLE_STR_ATI);
      ...
      glSampleMapATI(GL_REG_0_ATI, GL_REG_0_ATI, GL_SWIZZLE_STR_ATI);   //grab data
#if 0 
      glColorFragmentOp2ATI(GL_MUL_ATI, 
						GL_REG_0_ATI, GL_NONE, GL_SATURATE_BIT_ATI,
						GL_REG_0_ATI, GL_NONE, GL_NONE,
