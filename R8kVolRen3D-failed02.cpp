	...
	if(gluvv.light.showView){
		...
		glActiveTexture(GL_TEXTURE5_ARB); {
			...
			glEnable(GL_TEXTURE_2D);
				...
				wglBindTexImageARB(pbuff2->buffer, WGL_FRONT_LEFT_ARB, NULL);
				glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
				glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
...
void R8kVolRen3D::loadCubeTex(int good)      //updates cube map for shading
{
	...
	if(!good){
		pbuff->MakeCurrent(); { //render points to pbuffer
			...
			GlErr("R8kVolRen3D::loadCubeTex", " begin");
			...
			for(int cf=0; cf<6; ++cf){ //for each cube face (cf)
				GlErr("R8kVolRen3D::loadCubeTex", " render loop start");
				glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
				glColor4f(1,1,1,1);
				glBegin(GL_POINTS);{
					for(int i=0; i<csz; ++i){
				...
				GlErr("R8kVolRen3D::loadCubeTex", " render loop normals");
				...
				glCopyTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + cf, 
					0, 0, 0, 0, 0, csz, csz);
				glFlush();
				GlErr("R8kVolRen3D::loadCubeTex", " render loop copy");
				glBindTexture(GL_TEXTURE_CUBE_MAP_ARB, 0);			
				glDisable(GL_TEXTURE_CUBE_MAP_ARB);
				GlErr("R8kVolRen3D::loadCubeTex", " render loop end");
			...
			glBindTexture(GL_TEXTURE_2D, 0);
			glDisable(GL_TEXTURE_2D);
			GlErr("R8kVolRen3D::loadCubeTex", " light view");
#endif
			glMatrixMode(GL_PROJECTION);
			glPopMatrix();
			glMatrixMode(GL_MODELVIEW);
			...
			glDisable(GL_LIGHTING);
			GlErr("R8kVolRen3D::loadCubeTex", " end");
		} MakeCurrent();
...
#if 0
				glColorFragmentOp1ATI(GL_MOV_ATI,   //just for testing
					GL_REG_0_ATI, GL_NONE, GL_NONE,
