...
//-------------------------------------------------------------------------
// NV20VolRen3D3.cpp: implementation of the NV20VolRen3D3 class.
...
void NV20VolRen3D3::reshape(int w, int h)
{
	updateTBTex();
}
...
void NV20VolRen3D3::renderVolume()
{
	if(gluvv.picking){
		return;
	}
  ...
  //-------------- do dot product with clip and view dir
	...
	if(gluvv.light.showView){
		...
		glBindTexture(GL_TEXTURE_2D, shadowName[0]);
		glBegin(GL_QUADS);{
			glTexCoord2f(0,0);
			glVertex3f(0,0,0);
			glTexCoord2f(1,0);
...
void NV20VolRen3D3::renderBricks(GLdouble mv[16], GLfloat axis[4])
{
	
	if(gluvv.mv->numSubVols > 1){ //render multiple bricked volumes
		int *order = new int[gluvv.mv->numSubVols];   //the correct draw order
		int *lorder = new int[gluvv.mv->numSubVols];   //the correct draw order
		float *zval = new float[gluvv.mv->numSubVols];//zvalue of each subvolume
		float *lzval = new float[gluvv.mv->numSubVols];//zvalue of each subvolume
		float ref[3]; //view reference
		float lref[3]; //light reference
			copyV3(lref, gluvv.light.pos);
			scaleV3(100, lref);
		...
		for(i=0; i< gluvv.mv->numSubVols-1; ++i){ //now sort
			for(int j=i+1; j<gluvv.mv->numSubVols; ++j){
				...
				if(lzval[lorder[i]] > lzval[lorder[j]]){ //test for light swap (front to back)
					int tmp = lorder[i];
					lorder[i] = lorder[j];
					lorder[j] = tmp;
				}
		...
		for(i=0 ; i< gluvv.mv->numSubVols; ++i){ //finaly render
			...
			if(gluvv.light.shadow)
				v = &gluvv.mv->volumes[lorder[i]];
			else
				v = &gluvv.mv->volumes[order[i]];

			float sxf = v->xfSize;
			float syf = v->yfSize;
			float szf = v->zfSize;
			float hpx = 1.0/(v->xiSize*2.0);
			float hpy = 1.0/(v->yiSize*2.0);
			float hpz = 1.0/(v->ziSize*2.0);
			...
			glPushMatrix(); {
				if(gluvv.light.shadow){ //shadowing with bricks is a bitch
					setupClips(lorder[i],vo,tx); 	
					glTranslatef(v->xfPos, v->yfPos, v->zfPos);
					render3DVA(gluvv.volren.sampleRate,mv,lorder[i],vo,tx,axis);
					glActiveTexture(GL_TEXTURE0_ARB); { //this is for the slice
						glEnable(GL_TEXTURE_2D);
						glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);
						glTexEnvi(GL_TEXTURE_SHADER_NV, GL_SHADER_OPERATION_NV, GL_TEXTURE_2D);
						glBindTexture(GL_TEXTURE_2D, tbNames[lorder[i]]);
					} //end texture 2					
					glCopyTexSubImage2D(GL_TEXTURE_2D, 0, (int)0, (int)0, 
							                (int)0, (int)0,
							                (int)gluvv.win.width, (int)gluvv.win.height);
					glClear(GL_COLOR_BUFFER_BIT);
				} else { //normal brick rendering
...
//========================================================== Setup Clips
//======================================================================
void NV20VolRen3D3::setupClips(int vol, float vo[8][3], float tx[8][3])
{
...
//============================================================ drawClips
//======================================================================
void NV20VolRen3D3::drawClip(int vol, float dv, float wmv[16])
{
	if((!gluvv.clip.ortho)){
		switch(gluvv.dmode){
		case GDM_V1:
...
//======================================================================
void NV20VolRen3D3::renderSlice(float c[4][3], int vol)
{
	glActiveTexture(GL_TEXTURE0_ARB); { //this is for the slice
...
//======================================================== Setup Shaders
//======================================================================
void NV20VolRen3D3::setupShaders()
{
...
void NV20VolRen3D3::resetShaders()
{
...
void NV20VolRen3D3::setupRegComb(float ftb)
{
	if((gluvv.shade == gluvvShadeDiff) || (gluvv.shade == gluvvShadeDSpec)){
		float vdir[3];
...
void NV20VolRen3D3::resetRegComb()
{
...
//====================================================== Set Up P-Buffer
//======================================================================
void NV20VolRen3D3::setupPBuff()       //prepare pbuffer for rendering
{
...
void NV20VolRen3D3::render3DVA(float sampleFrequency,
		GLdouble mv[16],
		unsigned int v,
...
//standard renderer
inline void NV20VolRen3D3::volSlice(int edges, 
											             float tc[6][3], 
											             float pc[6][3],
											             int   order[6])
{
...
//render with shadows
inline void NV20VolRen3D3::volShadow(int edges,
																		 float tc[6][3],
																		 float pc[6][3],
																		 int   order[6],
																		 float ltxf[16],
...
inline int NV20VolRen3D3::intersect(
...
void NV20VolRen3D3::renderSlice()
{
	glPushMatrix();{
	...
	GlErr("NV20VolRen3D3", "Draw Slice");	
...
void NV20VolRen3D3::createVolumes()
{
	if(!glTexImage3DEXT){
		go = 0;
		return;
...
//======================================================= Create Bricks
//======================================================================
void NV20VolRen3D3::createBricks()
{
	...
	for(int v=0; v<gluvv.mv->numSubVols; ++v){
		...
		case GDM_VGH:
			...
			break;
		default:
			cerr << "Error NV20VolRen3D3::createBricks(), data type unknown" << endl;
			return;
		}
		...
		glFlush();
		GlErr("NV20VolRen3D3", "Create Texture - Shade Texture");
		cerr << "Done" << endl;
...
//=================================================== load 1 byte volume
//======================================================================
void NV20VolRen3D3::loadTex1B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		...
		glFlush();
		GlErr("NV20VolRen3D3", "Create Texture - Data Texture");	
		cerr << "Done" << endl;	
}

//=================================================== load 2 byte volume
//======================================================================
void NV20VolRen3D3::loadTex2B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		cerr << " Down Loading Data Texture " << name << " ... ";
		glEnable(GL_TEXTURE_3D);
		glBindTexture(GL_TEXTURE_3D, name);
...
//=================================================== load 4 byte volume
//======================================================================
void NV20VolRen3D3::loadTex4B(unsigned char *tex, unsigned int name, int sx, int sy, int sz)
{
		cerr << " Down Loading Data Texture " << name << " ... ";
		glEnable(GL_TEXTURE_3D);
		glBindTexture(GL_TEXTURE_3D, name);
...
void NV20VolRen3D3::create2DDepTex()
{
	//------------- load gb deptex --------------------
...
//========================================================= load Dep tex
//======================================================================
void NV20VolRen3D3::loadDepTex(unsigned char *dtex)
{
	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, deptexName);
...
void NV20VolRen3D3::loadDepTex(unsigned char *dtex, unsigned int name)
{
	glEnable(GL_TEXTURE_2D);
...
//==================================================== create Shadow tex
//======================================================================
void NV20VolRen3D3::createShadowTex()  //creates the shadow texture
{
	shadowName = new unsigned int[2];
	glGenTextures(2, shadowName);   //the dep tex that we use for the tf
	unsigned char *stex = new unsigned char[gluvv.light.buffsz[0]*gluvv.light.buffsz[1]*4];

	for(int i=0; i<2; ++i){
		glEnable(GL_TEXTURE_2D);
		glBindTexture(GL_TEXTURE_2D, shadowName[i]);		
		glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	...
	tbNames = new unsigned int[gluvv.mv->numSubVols];
	glGenTextures(gluvv.mv->numSubVols, tbNames);   //the dep tex that we use for the tf
	unsigned char *tbtex = new unsigned char[gluvv.win.height*gluvv.win.width*4];
	
	for(i=0; i<gluvv.mv->numSubVols; ++i){
		glEnable(GL_TEXTURE_2D);
		glBindTexture(GL_TEXTURE_2D, tbNames[i]);		
		glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
...
void NV20VolRen3D3::updateTBTex()      //update the temporary brick textures
{
	unsigned char *tbtex = new unsigned char[gluvv.win.height*gluvv.win.width*4];
...
void NV20VolRen3D3::scaleAlpha(float nsr, float osr, unsigned char *dtex)
{
	if(nsr == osr) return;
...
void NV20VolRen3D3::copyScale(float sr, unsigned char *dtex)
{
	int dsx = gluvv.tf.ptexsz[0];
	int dsy = gluvv.tf.ptexsz[1];
...
void NV20VolRen3D3::MakeCurrent()
{
