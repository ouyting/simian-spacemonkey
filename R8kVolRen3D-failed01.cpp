...
void R8kVolRen3D::init()
{
	...
	if(gluvv.mv){
		...
		if(!(pbuff = new PBuffer(gluvv.light.buffsz[0], gluvv.light.buffsz[1], 
			...
			pbuff->Initialize(true, false); //create a shared pbuffer, with render to texture
			pbuff->MakeCurrent(); {   
   ...
   for(i = 0 ; i < samples; ++i){ //for each slice
      ...
      if(edges == 3){ //no sort required for triangles
				...
				if(!gluvv.light.shadow)
					...
					//volShadR2T(edges,tcoord,poly,order,ltxf,texNames[v],mv);
#endif
				//glFlush();
																		 ...
																		 double mv[16])
{
	...
	for(int i=0; i<edges; ++i){
		...
		lc[idx][0] = (lv[idx][0]*xscale) + .5;
		lc[idx][1] = (lv[idx][1]*yscale) + .5;
	...
	pbuff->MakeCurrent();{
		...
		if(((maxx-minx)>1) && ((maxy-miny)>1)){
			glCopyTexSubImage2D(GL_TEXTURE_2D, 0, (int)(minx*gluvv.light.buffsz[0]), (int)(miny*gluvv.light.buffsz[1]), 
				                  (int)(minx*gluvv.light.buffsz[1]), (int)(miny*gluvv.light.buffsz[1]),
					                (int)((maxx-minx)*gluvv.light.buffsz[1]), (int)((maxy-miny)*gluvv.light.buffsz[1]));
		}
...
//render with shadows & render to texture ========================
inline void R8kVolRen3D::volShadR2T(int edges,         //the render to texture shadow
																		float tc[6][3],
																		float pc[6][3],
																		int   order[6],
																		float ltxf[16],
																		...
																		double mv[16])
{
	...
	//cerr << "light tf"; printMatrix(ltxf); cerr << endl;
	for(int i=0; i<edges; ++i){
		//move the coords into light space
		//first the vertices
		int idx = order[i];
		...
		lv[idx][1] /= lv[idx][3];
		//now the texture coords in screen space
		lc[idx][0] = (lv[idx][0]*.80 + .5);
		lc[idx][1] = (lv[idx][1]*.80 + .5);
		//get the light buffer coords (screen space) 
		lv[idx][0] = lc[idx][0];
		lv[idx][1] = lc[idx][1];

		if((lv[idx][0] > 1) || (lv[idx][1] > 1)){
			cerr << "bang funck" << endl;
			printV3(lv[idx]);
			cerr << endl;
		}
	}
